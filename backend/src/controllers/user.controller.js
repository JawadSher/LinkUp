...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    [firstName, lastName, email, channelName, password].some(
      (field) => field?.trim() === ""
    )
  ) {
  ...
  const avatar = avatarLocalPath
    ...
    : null;
  const bannerImage = bannerImageLocalPath
    ? await uploadtoCloudinary(bannerImageLocalPath)
    : null;
...
export const updateBannerImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Banner image updated successfully"));
});
...
export const getUserChannelProfile = asyncHandler(async (req, res) => {
  try {
    ...
    if (!req.user?._id || !moongose.Types.ObjectId.isValid(req.user?._id)) {
      throw new ApiError(400, "Unauthorized request or Invalid user ID");
    }
